...
- 反射词条效果不同，默认情况下反射3格内的所有类型伤害，该范围可以配置，也可以配置是否反射魔法伤害，另外可以限制反射伤害不超过目标最大生命值的reflectLimit倍
- 反射效果的实现不同，此处的逻辑：
...
- 摄魂词条：免疫物理伤害和伤害穿透都有冷却，可配置
- 破魔词条：免疫魔法伤害和伤害穿透都有冷却，封印的附魔数量为随机，最大数量正比于词条等级，可配置
